...
function FileViewer({ closeDialog, publicationData }) {
  ...
  console.log(closeDialog, publicationData, 'publicationData');
  ...
  const loadBravaViewer = useCallback(() => {
    axios
      .get(
        `https://na-1-dev.api.opentext.com/viewer/api/v1/viewers/brava-view-1.x/loader`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('opentext_token')}`,
          },
        },
      )
      .then((res) => {
  ...
  }, [localStorage.getItem('opentext_token')]);
  ...
  useEffect(() => {
    if (didMountRef.current) {
      if (bravaApi) {
        ...
        bravaApi.viewPublication(bravaApi.getActivePublication(), true);
